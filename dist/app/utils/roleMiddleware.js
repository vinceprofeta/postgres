...
function fetchMemberships() {
  return bookshelf.knex('memberships').join('roles', 'roles.id', '=', 'memberships.membership_role_id').where('membership_user_id', Number(5)).andWhere('role_name', 'resource-admin').select('*').then(function (membership) {
