...
// Actions
var StripeActions = require('./stripe');
...
transactions.getAll = function (limit, offset) {
  return Transactions.find({}).limit(limit || 10).skip(offset || 0).exec(function (err, transactions) {
    return transactions;
  });
};
...
transactions.getById = function (id) {
  return Transactions.findOne({
    _id: id
  }).populate('instructor userCharged session').deepPopulate('session.instructor').exec(function (err, transaction) {
    return transaction;
  });
};
...
transactions.getTransactions = function (id, limit, offset, start, end, isGym, onlyFailed, orInstructed, student, instructor) {
  ...
  if (isGym) {
    ...
    params.$or = [{ instructor: id }, { userCharged: id }];
  } else if (id) {
  ...
  return Transactions.find(params).limit(limit || 10).skip(offset || 0).populate('instructor userCharged session').deepPopulate('session.listing.instructor').sort({ date: 1 }).exec(function (err, sessions) {
    return sessions;
  });
};
...
transactions.getTransactionsForSession = function (sessionId, userId, notRefunded) {
  return new BluebirdPromise(function (resolve, reject) {
    ...
    Transactions.find(params).populate('userCharged').exec(function (err, transaction) {
      if (err) {
        reject(err);
      } else {
        resolve(transaction);
...
transactions.refundById = function (id, amount) {
  return new BluebirdPromise(function (resolve, reject) {
    Transactions.findOne({
      _id: id
    }).populate('session').exec(function (err, transaction) {
      if (err || !transaction) {
        throw err;
      } else {
...
transactions.refundUserForSession = function (transaction, session, amount) {
  return new BluebirdPromise(function (resolve, reject) {
    StripeActions.refundTransaction(transaction.stripe.id, amount, session, transaction.userCharged).then(function (refund) {
