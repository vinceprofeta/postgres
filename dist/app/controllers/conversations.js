...
rooms.getByUsersInConversation = function (userIds) {
  return new BluebirdPromise(function (resolve, reject) {
    ...
    bookshelf.knex('usersConversations').select('conversation_id').whereIn('conversation_user_id', ids).groupBy('conversation_id').havingRaw('count(*) = ?', [ids.length]).then(function (conversation) {
      if (!_.get(conversation, 'length')) {
        return rooms.add({
          userOne: ids[0],
          userTwo: ids[1]
        }).then(function () {
          return bookshelf.knex('usersConversations').select('conversation_id').whereIn('conversation_user_id', ids).groupBy('conversation_id').havingRaw('count(*) = ?', [ids.length]).then(function (conversation) {
...
rooms.updateById = function (id, params) {
  return bookshelf.knex('conversations').where('id', '=', id).update({
